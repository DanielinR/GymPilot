...
export default function RoutinePage({
    ...
    const deleteExercise = async (exercise: Exercise) => {
        if (!exercise) return;
        await setJsonFromAPI("/trainingTemplates/" + params.routine + "/deleteExercise", exercise)
        updateExercises()
    }
    const addExercise = async (exercise: Exercise) => {
        setViewSelectExercise(false);
        if (!exercise) return;
        await setJsonFromAPI("/trainingTemplates/" + params.routine + "/addExercise", exercise)
        updateExercises()
    }
    async function updateExercises() {
        const response = await getJsonFromAPI("/trainingTemplates/" + params.routine, "exercises");
        setExercises(response);
    }
    useEffect(() => {
        async function updateName() {
            const response = await getJsonFromAPI("/trainingTemplates/" + params.routine, "name");
            setName(response);
        }
        updateExercises();
        updateName();
    ...
    return (
        <div className="h-full w-full flex items-center justify-center overflow-hidden flex-col text-white relative pt-6 md:pt-24 gap-8 md:gap-14">
            ...
            ></DeleteModal>
            {viewSelectExercise && <div className={`absolute top-0 z-50 bg-neutral-500 bg-opacity-80 p-5 flex items-center w-[90%] h-[95%] rounded-lg`}>
                <List<Exercise>
                    tittle={"Add exercise"}
                    tittleSize="text-4xl"
                    searchBy="name"
                ...
                ></List>
                <button onClick={()=>{setViewSelectExercise(false)}}><XIcon className="h-5 w-5 absolute top-3 right-3"></XIcon></button>
            </div>}
            ...
            <div className="flex flex-col w-full h-full gap-3 mb-10 items-center overflow-hidden">
                <button onClick={() => { setViewSelectExercise(true) }} className="p-3 bg-brand-500 hover:bg-brand-700 rounded-full shadow-lg">
                ...
                <div className="flex flex-1 w-[75%] flex-col items-center h-full relative gap-5 overflow-auto pt- p-7 mb-7">
                    {exercises?.map((item) => {
                        return (
                            <div key={item.id} className="bg-neutral-500 bg-opacity-90 p-5 w-full rounded-lg flex items-center justify-center text-xl font-bold relative">
                                    {item.name}
                                    <button
                                        onClick={() => {
                                            deleteExercise(item)
